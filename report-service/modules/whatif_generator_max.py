...
# nest_asyncio.apply() 제거: FastAPI BackgroundTasks의 worker 스레드에서는
# 새 이벤트루프를 만들면 되므로 불필요. import 시점 적용 시 uvicorn 메인 루프를
# 깨뜨려 anyio.NoEventLoopError 발생함.
...
class WhatIfGeneratorMax:
    ...
    async def _async_generate(self, route, ice_class, departure_date, forecast_days):
        ...
        logger.info("[v3.2] Claude SDK query 시작")
        chunks = []
        msg_count = 0
        async for message in query(prompt=prompt, options=options):
            msg_count += 1
            if msg_count == 1:
                logger.info("[v3.2] 첫 메시지 수신")
            if isinstance(message, AssistantMessage):
                for block in message.content:
                    if isinstance(block, TextBlock):
                        chunks.append(block.text)
        logger.info("[v3.2] Claude SDK query 종료 — 총 %d 메시지, 텍스트 청크 %d",
    ...
    def generate_scenarios(self, route="NSR", ice_class="PC5",
        ...
        logger.info("[v3.2] generate_scenarios 시작 route=%s ice=%s days=%d",
                    route, ice_class, forecast_days)

        # FastAPI BackgroundTasks의 worker 스레드에서 호출되므로
        # 이벤트루프가 없는 깨끗한 상태. asyncio.run()으로 새 루프 생성/정리.
        ai_text = asyncio.run(
            self._async_generate(route, ice_class, departure_date, forecast_days)
        )

        logger.info("[v3.2] _async_generate 완료, 텍스트 길이=%d, 도구호출=%d",
                    len(ai_text), self.tool_calls_count)
