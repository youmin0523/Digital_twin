...
// [설정] 스케줄러 및 데이터 환경변수
const CONFIG = {
  COPERNICUS_THREDDS_URL:
    process.env.COPERNICUS_URL ||
    'https://my.cmems-du.eu/thredds/ncss/grid/...',
  NSIDC_API_URL:
    process.env.NSIDC_URL || 'https://nsidc.org/api/mapservices/NSIDC/wms',
...
function parseNetCDFMock() {
  console.log(
    '[Proxy] 외부 API 호출: Copernicus NetCDF / NSIDC 데이터 다운로드 중... (Mock)',
  );
  const cells = [];
...
async function runBatchPipeline(targetDateStr = null) {
  try {
    ...
    if (fs.existsSync(cacheFile)) {
      console.log(
        `[Cache Hit] 해당 날짜(${targetDate})의 파싱된 데이터가 캐시에 이미 존재합니다. API 호출을 생략합니다.`,
      );
    ...
    console.log(
      `[Cache Miss] 해당 날짜(${targetDate}) 데이터가 없습니다. 외부 API를 호출하여 파싱을 시작합니다...`,
    );

    // 1. 외부 API 연동 및 파싱 (NetCDF -> Memory Array)
    const parsedGrid = parseNetCDFMock();
    console.log(
      `[Proxy] 파싱 완료. 생성된 의미있는 격자 수: ${parsedGrid.length} 개 셀`,
    );
    ...
    console.log(
      `[Proxy] JSON 생성 및 캐시 저장 성공. 소요시간: ${elapsed.toFixed(2)}초`,
    );
