...
def read_wi():
    p = Path("c:/cccc/Digital_twin/report-service/data/whatif_iterative_history.json")
    if not p.exists():
        return 0, 0
    try:
        d = json.loads(p.read_text(encoding="utf-8"))
...
def read_rl_files():
    files = list(Path("c:/cccc/Digital_twin/rl-pipeline/models").glob("iterative_history_*.json"))
    ti = 0; cv = 0
    for f in files:
        try:
...
def read_ct_log():
    p = Path("c:/cccc/Digital_twin/logs/continuous_trainer.log")
    if not p.exists():
        return "로그 없음"
    try:
...
def report():
    ...
    rp_files2 = list(Path("c:/cccc/Digital_twin/report-service/data").glob("departure_iterative_history_*.json"))
    sar_iters, sar_best, _ = read_history("c:/cccc/Digital_twin/backend/pipeline/models/iceberg_iterative_history.json", "mAP50")
    fuel_iters, fuel_best, _ = read_history("c:/cccc/Digital_twin/ml-pipeline/models/fuel_iterative_history.json", "R2")
